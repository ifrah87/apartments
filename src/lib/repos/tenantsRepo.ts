...
function isMissingTenantsTable(err: unknown) {
  const code = (err as { code?: string })?.code;
  const message = err instanceof Error ? err.message : String(err);
  return code === "42P01" || message.includes('relation "tenants" does not exist');
}
...
async function withTenantsTable<T>(operation: () => Promise<T>): Promise<T> {
  try {
    return await operation();
  } catch (err) {
    if (isMissingTenantsTable(err)) {
