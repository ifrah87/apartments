...
export default async function UnitFinancialReportPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  const sp = await searchParams;
  const properties = await fetchPropertyOptions();
  const defaults = defaultRange();
  const start = sp.start || defaults.start;
  const end = sp.end || defaults.end;
  const report = await buildUnitFinancialReport({ propertyId: sp.property, start, end }, properties);
